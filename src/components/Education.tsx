        ...
        <div className="relative">
          ...
          {educationData.map((item, index) => (
            <motion.div 
              ...
              className={`relative mb-20 flex items-center ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}
            >
              {/* Timeline dot */}
              <div className="absolute left-1/2 transform -translate-x-1/2 w-5 h-5 rounded-full bg-primary border-2 border-background z-10"></div>
              
              {/* Content card with year badge as part of the card, not floating */}
              <div className={`w-5/12 ${index % 2 === 0 ? 'pr-12' : 'pl-12'}`}>
                {applyGlass(
                  <div className="relative">
                    {/* Year badge - now part of the card, not floating */}
                    <div className="bg-gradient-to-r from-primary to-secondary text-white py-3 px-4 rounded-t-lg text-center font-medium">
                      {item.year}
                    </div>
